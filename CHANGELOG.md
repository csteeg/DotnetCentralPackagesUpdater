...
- **Package Version Freezing**: Added `FreezeVersion="true"` attribute support on `PackageVersion` elements to exclude specific packages from automatic updates. Packages with this attribute are skipped during update checks and displayed in a separate "Packages excluded from updates" section.
